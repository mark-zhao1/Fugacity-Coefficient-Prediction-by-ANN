...
Plot phase split compositions, EOS vs ANN. This is implemented in Jupyter Notebook
PycharmProjectsLn_phi_modelApplied_modelnC4-nC10_evaluationPhase Split Comparison
...
'''
    ...
    print(sumXX_list)
    #print('Liq tmp: {}'.format(pr.liq_tmp))
    #print('Vap tmp: {}'.format(pr.vap_tmp))
    '''# Plot TPD
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.plot(pr.liq_it, pr.liq_tmp, label='liq')
    ax.plot(pr.vap_it, pr.vap_tmp, label='vap')
    ax.set_xlabel('Iteration')
    ax.set_ylabel('Residual')
    ax.set_yscale('log')
    ax.set_title('SA stationarity equation residual per iteration, ANN model')
    ax.legend()
    plt.show()'''
